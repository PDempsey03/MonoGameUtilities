using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Content;
using Microsoft.Xna.Framework.Graphics;
using System;
using System.IO;

namespace Mmc.MonoGame.Collisions.GameTests
{
    public static class Drawer
    {
        private static Texture2D _pixel;
        private static SpriteFont _font;

        public static void Initialize(GraphicsDevice graphicsDevice)
        {
            _pixel = new Texture2D(graphicsDevice, 1, 1);
            _pixel.SetData([Color.White]);
        }

        public static void LoadContent(ContentManager content)
        {
            _font = content.Load<SpriteFont>(Path.Combine("Fonts", "Asap"));
        }

        public static void DrawLine(Vector2 start, Vector2 end, Color color, float thickness = 1)
        {
            Vector2 line = end - start;
            float angle = MathF.Atan2(line.Y, line.X);

            Globals.SpriteBatch.Draw(_pixel, start, null, color, angle, Vector2.Zero, new Vector2(line.Length(), thickness), SpriteEffects.None, 0);
        }

        public static void DrawRectangle(Vector2 vertex1, Vector2 vertex2, Vector2 vertex3, Vector2 vertex4, Color color, float thickness = 1)
        {
            DrawLine(vertex1, vertex2, color, thickness);
            DrawLine(vertex2, vertex3, color, thickness);
            DrawLine(vertex3, vertex4, color, thickness);
            DrawLine(vertex4, vertex1, color, thickness);
        }

        public static void DrawCircle(Vector2 center, float radius, Color color, int samplePoints = 50, float thickness = 1, float startingAngleRadians = 0, float endingAngleRadians = MathF.Tau)
        {
            float currentAngle = startingAngleRadians;
            float deltaAngle = (endingAngleRadians - startingAngleRadians) / samplePoints;

            Vector2 currentPoint;
            Vector2 previousPoint = center + radius * new Vector2(MathF.Cos(currentAngle), MathF.Sin(currentAngle));
            currentAngle += deltaAngle;

            for (int i = 1; i <= samplePoints; i++)
            {
                currentPoint = center + radius * new Vector2(MathF.Cos(currentAngle), MathF.Sin(currentAngle));

                DrawLine(previousPoint, currentPoint, color, thickness);

                previousPoint = currentPoint;
                currentAngle += deltaAngle;
            }
        }

        /// <summary>
        /// Draws text to the screen
        /// </summary>
        /// <param name="text">Text to be drawn to the screen.</param>
        /// <param name="position">Top left position of the text (i.e. text is drawn below and right of this position)</param>
        /// <param name="color">Color of the text.</param>
        public static void DrawText(string text, Vector2 position, Color color)
        {
            Globals.SpriteBatch.DrawString(_font, text, position, color);
        }
    }
}
